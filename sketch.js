...
function draw() {
  ...
  if (gamestate === "end") {
    ...
    stroke("pink");
    strokeWeight(1)
    text("Uh-oh! The pumpkins have take over!", width/2, height/2);
    text("Don't worry, we can try again! Press R to play", width/2, height/2+20)
    luna.destroy();
   // bgsprite.destroy();
    ...
    if (mouseIsOver(endpumpkin)) {
      text("You cannot defeat me! MWHAHAHAHHA!", width/2-50, height/4)
    }
  }
  textSize(12);
  stroke("white")
